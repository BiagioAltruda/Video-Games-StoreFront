...
(function () {                     // IIFE = funzione auto-eseguita: isola le variabili dallo scope globale
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!Number.isInteger(myId) || myId <= 0) {
      showMsg('warning', 'Manca il tuo ID utente. (myUserId)');
      return;
    }
    ...
    try {
      ...
      showMsg('success', `Amico (ID ${friendId}) aggiunto con successo ✅`);
      friendEl.value = '';
      updateBtnState();
      ...
      showMsg('danger', 'Errore aggiunta amico: ' + err.message);
      console.error(err);
    } finally {
